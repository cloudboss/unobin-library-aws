...
import (
	...
	lmvms "github.com/cloudboss/unobin-library-aws/internal/service/lambdamicrovms"
)
...
func TestLibraryRegistersLambdaMicrovms(t *testing.T) {
	...
	resources := []constructRegistration{
		{
			key:        "lambdamicrovms-microvm-image",
			outputType: reflect.TypeFor[*lmvms.MicrovmImageOutput](),
		},
	}
	...
	dataSources := []constructRegistration{
		{
			key:        "lambdamicrovms-microvm-image",
			outputType: reflect.TypeFor[*lmvms.MicrovmImageDataOutput](),
		},
		{
			key:        "lambdamicrovms-microvm-images",
			outputType: reflect.TypeFor[*lmvms.MicrovmImagesOutput](),
		},
		{
			key:        "lambdamicrovms-microvm-image-version",
			outputType: reflect.TypeFor[*lmvms.MicrovmImageVersionDataOutput](),
		},
		{
			key:        "lambdamicrovms-microvm-image-versions",
			outputType: reflect.TypeFor[*lmvms.MicrovmImageVersionsOutput](),
		},
		{
			key:        "lambdamicrovms-microvm-image-build",
			outputType: reflect.TypeFor[*lmvms.MicrovmImageBuildDataOutput](),
		},
		{
			key:        "lambdamicrovms-microvm-image-builds",
			outputType: reflect.TypeFor[*lmvms.MicrovmImageBuildsOutput](),
		},
		{
			key:        "lambdamicrovms-managed-microvm-images",
			outputType: reflect.TypeFor[*lmvms.ManagedMicrovmImagesOutput](),
		},
		{
			key:        "lambdamicrovms-managed-microvm-image-versions",
			outputType: reflect.TypeFor[*lmvms.ManagedMicrovmImageVersionsOutput](),
		},
		{
			key:        "lambdamicrovms-microvm",
			outputType: reflect.TypeFor[*lmvms.MicrovmDataOutput](),
		},
		{
			key:        "lambdamicrovms-microvms",
			outputType: reflect.TypeFor[*lmvms.MicrovmsOutput](),
		},
	}
	...
	actions := []constructRegistration{
		{
			key:        "lambdamicrovms-run-microvm",
			outputType: reflect.TypeFor[*lmvms.MicrovmDataOutput](),
		},
		{
			key:        "lambdamicrovms-create-microvm-auth-token",
			outputType: reflect.TypeFor[*lmvms.MicrovmAuthTokenOutput](),
		},
		{
			key:        "lambdamicrovms-create-microvm-shell-auth-token",
			outputType: reflect.TypeFor[*lmvms.MicrovmShellAuthTokenOutput](),
		},
		{
			key:        "lambdamicrovms-suspend-microvm",
			outputType: reflect.TypeFor[*lmvms.SuspendMicrovmOutput](),
		},
		{
			key:        "lambdamicrovms-resume-microvm",
			outputType: reflect.TypeFor[*lmvms.ResumeMicrovmOutput](),
		},
		{
			key:        "lambdamicrovms-terminate-microvm",
			outputType: reflect.TypeFor[*lmvms.TerminateMicrovmOutput](),
		},
		{
			key:        "lambdamicrovms-update-microvm-image-version-status",
			outputType: reflect.TypeFor[*lmvms.MicrovmImageVersionDataOutput](),
		},
	}
...
func TestLambdaMicrovmSchemas(t *testing.T) {
	...
	}, schema.Resources["lambdamicrovms-microvm-image"])

	assert.Equal(t, []string{"name"}, (&lmvms.MicrovmImage{}).ReplaceFields())
	...
	}, schema.DataSources["lambdamicrovms-microvm-image"])
	...
	}, schema.DataSources["lambdamicrovms-microvm-images"])
	...
	}, schema.DataSources["lambdamicrovms-microvm-image-version"])
	...
	}, schema.DataSources["lambdamicrovms-microvm-image-versions"])
	...
	}, schema.DataSources["lambdamicrovms-microvm-image-build"])
	...
	}, schema.DataSources["lambdamicrovms-microvm-image-builds"])
	...
	}, schema.DataSources["lambdamicrovms-managed-microvm-images"])
	...
	}, schema.DataSources["lambdamicrovms-managed-microvm-image-versions"])
	...
	}, schema.DataSources["lambdamicrovms-microvm"])
	...
	}, schema.DataSources["lambdamicrovms-microvms"])
	...
	assertSchemaFieldsEqual(t, &runtime.TypeSchema{
		...
		Outputs: microvmOutputTypes(),
	}, schema.Actions["lambdamicrovms-run-microvm"])
	...
	}, schema.Actions["lambdamicrovms-create-microvm-auth-token"])
	...
	assertSchemaFieldsEqual(t, &runtime.TypeSchema{
		...
		SensitiveOutputs: []string{"auth-token"},
	}, schema.Actions["lambdamicrovms-create-microvm-shell-auth-token"])

	for _, key := range []string{
		"lambdamicrovms-suspend-microvm",
		"lambdamicrovms-resume-microvm",
		"lambdamicrovms-terminate-microvm",
	} {
		assertSchemaFieldsEqual(t, &runtime.TypeSchema{
			Inputs: map[string]typecheck.Type{
				"microvm-identifier": typecheck.TString(),
	...
	assertSchemaFieldsEqual(t, &runtime.TypeSchema{
		...
		Outputs: microvmImageVersionOutputTypes(),
	}, schema.Actions["lambdamicrovms-update-microvm-image-version-status"])

	assert.Empty(t, schema.Resources["lambdamicrovms-microvm-image"].SensitiveInputs)
	assert.Empty(t, schema.Resources["lambdamicrovms-microvm-image"].SensitiveOutputs)
	...
	for key, sch := range schema.Actions {
		...
		assert.Empty(t, sch.SensitiveInputs, key)
		if key == "lambdamicrovms-create-microvm-auth-token" ||
			key == "lambdamicrovms-create-microvm-shell-auth-token" {
...
func assertSchemaConstraints(t *testing.T, schema *runtime.LibrarySchema) {
	t.Helper()
	image := schema.Resources["lambdamicrovms-microvm-image"]
	...
	assertConstraintsContain(t,
		schema.DataSources["lambdamicrovms-microvm-image"].Constraints,
		lang.ConstraintSpec{
			Kind: "exactly-one-of",
			Fields: []string{
	...
	assertConstraintMessage(t,
		schema.DataSources["lambdamicrovms-microvm-image-builds"].Constraints,
		"architecture must be ARM_64")
	assertConstraintMessage(t,
		schema.DataSources["lambdamicrovms-microvm-image-builds"].Constraints,
		"chipset must be GRAVITON")

	run := schema.Actions["lambdamicrovms-run-microvm"]
	...
	auth := schema.Actions["lambdamicrovms-create-microvm-auth-token"]
	...
	shellAuth := schema.Actions["lambdamicrovms-create-microvm-shell-auth-token"]
	assertConstraintMessage(t, shellAuth.Constraints,
		"expiration-in-minutes must be between 1 and 60")

	status := schema.Actions["lambdamicrovms-update-microvm-image-version-status"]
